...
canonical-url: https://docs.6mm.com/zh/sdk/agent-sdk/java/embed-token
---
...
- [Trading Widget 身份认证](/zh/sdk/trading-widget/authentication)：把 `tokenProvider` 接入合作方后端接口。
- [Java 快速开始](/zh/sdk/agent-sdk/java/quick-start)：安装并初始化 `AgentClient`。
- [密钥与签名](/zh/sdk/security/secrets-signing)：保护 Agent API 密钥。
- [Webhook 与幂等](/zh/sdk/security/webhooks-idempotency)：保护接入流程的回调端。
